...
SELECT
    ...
    CASE
        ...
        ) THEN 'América do Norte'

        WHEN ct.nome_pais_origem IN (
            'Costa Rica', 'Cuba', 'El Salvador', 'Guatemala',
            'Haiti', 'Honduras', 'Nicarágua', 'Panamá',
            'República Dominicana', 'Trinidad e Tobago'
        ) THEN 'América Central e Caribe'

        WHEN ct.nome_pais_origem IN (
            'Alemanha', 'Áustria', 'Bélgica', 'Bulgária', 'Croácia',
            'Dinamarca', 'Eslováquia', 'Eslovênia', 'Espanha', 'Estônia',
            'Finlândia', 'França', 'Grécia', 'Holanda', 'Hungria',
            'Irlanda', 'Itália', 'Letônia', 'Lituânia', 'Luxemburgo',
            'Noruega', 'Polônia', 'Portugal', 'Reino Unido',
            'República Tcheca', 'Romênia', 'Rússia', 'Sérvia',
            'Suécia', 'Suíça', 'Turquia', 'Ucrânia'
        ) THEN 'Europa'

        WHEN ct.nome_pais_origem IN (
            'Arábia Saudita', 'Bangladesh', 'China', 'China, Hong Kong',
            'Cingapura', 'Filipinas', 'Índia', 'Indonésia', 'Irã',
            'Israel', 'Japão', 'Líbano', 'Malásia', 'Paquistão',
            'República da Coreia', 'Síria', 'Taiwan', 'Tailândia'
        ) THEN 'Ásia'

        WHEN ct.nome_pais_origem IN (
            'África do Sul', 'Angola', 'Cabo Verde', 'Egito',
            'Gana', 'Quênia', 'Marrocos', 'Moçambique',
            'Nigéria', 'Tunísia'
        ) THEN 'África'
...
SELECT continente, SUM(chegadas) AS total_turistas
FROM vw_continente_turistas
WHERE continente = 'Europa'
GROUP BY continente;

SELECT regiao, SUM(chegadas) AS total_turistas
FROM vw_regiao_turistas
